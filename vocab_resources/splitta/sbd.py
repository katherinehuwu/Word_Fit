...
SVM_CLASSIFY = '/usr/local/bin/svm_classify'
...
def is_sbd_hyp(word):
    ...
    if not any(char in word for char in ['.', '?', '!']): return False
    c = unannotate(word)
    if c.endswith('.') or c.endswith('?') or c.endswith('!'): return True
    if re.match(r'.*[.?!]+["\')\]]*$', c): return True
    return False
